...
// ---------------------------------------------------------------------------
// KYC helpers — gallery upload (ID docs) + mandatory live camera capture
// ---------------------------------------------------------------------------
...
window.kycCaptureLive = async (hiddenId, previewId, statusId, nextSectionId = '', facing = 'environment', title = 'Camera') => {
  ...
  const dataUrl = await openLiveCamera({
    ...
    maxEdge: isSelfie ? 900 : 1600, quality: 0.85
  })
  if (!dataUrl) return
  kycApplyCapture(dataUrl, hiddenId, previewId, statusId, nextSectionId)
}
...
function authSignUpVerify(phone, name, demoOtp) {
  ...
  $('authBody').innerHTML = `
    ...
    <form id="suvForm" class="space-y-4">
      ...
      <div class="pt-1"><h4 class="text-sm font-semibold text-teal-700 mb-2"><i class="fas fa-user mr-1"></i>Personal Information</h4>
        <div class="grid grid-cols-2 gap-2 text-sm">
          <input id="su_nid" type="text" inputmode="numeric" placeholder="National ID *" required class="px-3 py-2 border border-slate-300 rounded-lg col-span-2">
      ...
      <div class="pt-1"><h4 class="text-sm font-semibold text-teal-700 mb-2"><i class="fas fa-leaf mr-1"></i>Farming Profile</h4>
        <div class="grid grid-cols-2 gap-2 text-sm">
          <select id="vct" onchange="updateChain()" class="px-3 py-2 border border-slate-300 rounded-lg"><option value="">Value Chain Type</option><option value="crop">Crop</option><option value="livestock">Livestock</option></select>
          <select id="vc" class="px-3 py-2 border border-slate-300 rounded-lg"><option value="">Select type first</option></select>
  ...
  $('suvForm').onsubmit = async (e) => {
    ...
    if (!county) return toast('County is required', false)
    try {
      const { data } = await api.post('/signup/verify', {
        ...
        value_chain_type: ($('vct').value || '').trim(),
        value_chain: ($('vc').value || '').trim(),
...
window.payModal = async (id, amount, outstanding, kind, opts) => {
  ...
  showModal(`<h3 class="text-lg font-bold mb-1"><i class="fas fa-mobile-alt text-teal-600 mr-2"></i>${esc(title)}</h3>
    ...
    <!-- Dynamic legal agreement block — the terms shown here are loaded from the
         ...
         (cash / financing type). End-users only ever see the terms relevant to
    ...
    <div id="payTermsBlock" class="text-left mb-4 mt-1 p-3 bg-slate-50 border border-slate-200 rounded-lg">
      <div class="text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-1 text-left">${isCash ? 'Cash Sale Agreement' : 'Sale Agreement'}</div>
...
async function loadPayTerms(id, isCash) {
  ...
  const fallback = isCash
    ? 'This is an outright cash sale. By proceeding you confirm settlement of the amount due and accept transfer of ownership upon payment.'
    : 'This is a financing transaction. By proceeding you agree to the disclosed financing price, deposit and installment schedule until the balance is fully settled.'
  try {
...
// Build the inline CSS for the agreement body from the configured style
// (default: justified, 12pt Calibri, 1.5 line spacing).
...
function agreementTermsBody(ag) {
  ...
  if (isUpload && url) {
    ...
    const isWord = /^data:application\/(msword|vnd\.openxmlformats-officedocument\.wordprocessingml\.document)/i.test(url) || /\.(docx?|)($|\?)/i.test(url) && /msword|wordprocessing/i.test(url)
    if (isPdf) {
...
function productForm(prefix, p = {}) {
  ...
  return `
    ...
    <div class="responsive-grid cols-2 text-sm">
      ...
      <div><label class="field-label">Financing type</label><select id="${prefix}_fin_type" ${finDis} class="px-3 py-2 border rounded-lg ${finDis ? 'bg-slate-100 text-slate-400' : ''}" data-selected="${esc(p.financing_type_key || p.financing_model || '')}">
      ...
      <div style="grid-column:1 / -1" class="border rounded-xl p-3 bg-white">
        ...
        <div class="text-[11px] text-slate-400 mt-2">Leave rate/amount at 0 to keep the legacy min/max-months model below.</div>
      </div>
      ...
      ${agreementSourceBlock(prefix, 'fin', 'Financing / PAYGO agreement', p, finDis)}
    </div>`
}
...
window.pickImage = async (input, targetId, previewId) => {
  ...
  try {
    const raw = await kycFileToDataUrl(file)
    // Compress + optimize the product image while preserving clarity.
  ...
  } catch (_) { toast('Could not read the selected image', false) }
}
...
async function viewCustomers() {
  ...
  $('content').innerHTML = `${actionBar}<div class="card table-card"><table class="w-full text-sm">
    ...
    <tbody>${_customers.map(c => `<tr class="border-t border-slate-100">
      ...
      </td></tr>`).join('') || '<tr><td colspan="9" class="text-center py-8 text-slate-400">No customers</td></tr>'}</tbody>
  </table></div>`
}
...
// ---------------------------------------------------------------------------
// PAYMENT TENANTS (Admin-facing) — dynamic provisioning of client tenants
// (Credit / Feed) that delegate their financial transactions to this central
// payment gateway. Operators can register / edit a tenant, update its webhook,
// rotate its HMAC secret and toggle its active status WITHOUT a restart.
// Backs GET/POST /api/v1/admin/tenants and the rotate-secret / status endpoints.
// The full HMAC secret is only ever revealed at the moment it is minted/rotated.
// ---------------------------------------------------------------------------
...
window.pickAvatarFile = async (input) => {
  const file = input.files?.[0]
  if (!file) return
  ...
  } catch (_) { toast('Could not read the image file.', false) }
}
  ...
  $('content').innerHTML = `
    <div class="space-y-6 max-w-3xl">
      ...
      <div class="card p-6" id="finTypesCard">
        ...
        <p class="text-xs text-slate-500 mb-4">Create, edit or remove the financing options offered across the platform. Any active type becomes selectable during inventory listing and drives its own checkout agreement.</p>
...
function finTypeModal(t) {
  const editing = !!t
  t = t || { label: '', description: '', charge_mode: 'percentage', active: 1 }
...
window.saveFinType = async (id) => {
  const payload = {
    ...
    charge_mode: ($('ft_charge') || {}).value || 'percentage',
