-- =====================================================================
-- 0035 — Agreement source mode (Upload Document vs Type Agreement)
--
-- When listing inventory + setting up cash / financing terms, the person
-- adding the agreement can now CHOOSE, via a dropdown, HOW the agreement is
-- supplied for each payment path:
--
...
--
-- We persist the chosen mode per path so checkout knows which representation
-- to render. Existing rows are backfilled: if a doc URL is present the mode is
-- 'upload', otherwise 'editor' (preserving current behaviour).
--
...
-- Backfill from whatever the row already has so behaviour is unchanged.
UPDATE products
